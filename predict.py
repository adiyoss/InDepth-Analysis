...
model_path = "models/enc_dec_regression/first.word.enc.dec.reg.1000.model.net"
data_dir = "data/idx/first_word/"
rep_dir = "data/enc_dec_regression/"
order_path = "data/idx/order/test.txt.order.txt"
task = Tests.FIRST_WORD
...
print("Accuracy on the test set: %s" % (float(np.sum(y_hat == np.argmax(y_val, axis=1))) / len(y_hat)))
print(confusion_matrix(np.argmax(y_val, axis=1), y_hat))
